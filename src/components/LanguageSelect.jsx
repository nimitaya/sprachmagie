...
const LanguageSelect = () => {
  ...
  return (
    <div className="p-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
      <p className="uppercase text-2xl text-center font-semibold text-indigo-400 bg-amber-100 py-1 px-2 rounded-full">
        Sprache
      </p>
      <div ref={dropdownRef} className="dropdown inline-block relative">
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="bg-purple-700 text-gray-100 text-2xl font-semibold min-w-44 py-2 px-4 rounded inline-flex justify-between items-center cursor-pointer hover:bg-purple-600 transition"
        >
        ...
        <ul
          className={`dropdown-menu absolute min-w-44 ${
            isOpen ? "" : "hidden"
          } text-indigo-400 text-2xl font-semibold pt-1 z-50`}
        >
